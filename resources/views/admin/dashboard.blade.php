...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
    @php
        $cards = [
            ...
            ['label' => 'Data TTM',          'value' => \App\Models\Sunrise::count(),         'color' => 'amber',  'icon' => 'M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707M17.657 17.657l-.707-.707M6.343 6.343l-.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z'],
        ];
        $colorMap = [
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div class="bg-white rounded-2xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-2 gap-3">
            @php
                $links = [
                    ['href' => route('admin.staff.create'),             'label' => '+ Pegawai Baru',    'color' => 'bg-blue-600'],
                    ['href' => route('admin.buletin.create'),           'label' => '+ Buletin Baru',    'color' => 'bg-indigo-600'],
                    ['href' => route('admin.artikel.create'),           'label' => '+ Artikel Baru',    'color' => 'bg-teal-600'],
                    ['href' => route('admin.sunrise.create'),           'label' => '+ Data TTM',        'color' => 'bg-amber-500'],
                    ['href' => route('admin.lightning.create'),         'label' => '+ Data Petir',      'color' => 'bg-yellow-500'],
                    ['href' => route('admin.informasi-publik.create'),  'label' => '+ Berita/Pengumuman','color' => 'bg-rose-500'],
                ];
            @endphp
            @foreach($links as $link)
                <a href="{{ $link['href'] }}"
                   class="{{ $link['color'] }} text-white text-sm font-medium rounded-lg px-4 py-2.5 hover:opacity-90 transition-opacity text-center">
                    {{ $link['label'] }}
                </a>
            @endforeach
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6">
        ...
        <div class="space-y-3 text-sm">
            ...
            <div class="flex justify-between items-center py-2">
                <span class="text-gray-500">Berita & Pengumuman</span>
                <span class="font-medium text-gray-800">{{ \App\Models\InformasiPublik::count() }}</span>
            </div>
        </div>
    </div>
</div>
